...
def build_excel_download(data: pd.DataFrame, asset: str, ticker: str, source: str, window: int, decay: float) -> bytes:
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter", datetime_format="dd-mmm-yyyy") as writer:
        ...
        lessons = [
            ...
            ("Historic-volatility downside", "A single backward-looking standard deviation weights old and recent observations equally, reacts slowly to regime shifts, and does not by itself capture fat tails, asymmetry or changing conditional risk in VaR and ES."),
        ]
        ...
        guide.merge_range("A12:B12", "Important: volatility measures dispersion—not direction, expected return, Value at Risk, or maximum possible loss.", note_fmt)
