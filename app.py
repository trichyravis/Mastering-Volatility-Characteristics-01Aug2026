...
def build_excel_download(data: pd.DataFrame, asset: str, ticker: str, source: str, window: int, decay: float) -> bytes:
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter", datetime_format="dd-mmm-yyyy") as writer:
        ...
        summary.set_row(0, 30); summary.merge_range("A1:F1", "Understanding Characteristics of Volatility", title)
        ...
        guide.merge_range("A1:B1", "Characteristics of Volatility · Learning Guide", title)
        ...
        book.set_properties({"title": "Understanding Characteristics of Volatility", "author": "Mountain Path Academy", "comments": "Generated by the Streamlit learning studio"})
...
with tabs[9]:
    ...
    st.download_button("Download formatted Excel workbook", workbook_bytes, file_name=f"{ticker.replace('^','')}_volatility_characteristics.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
